...
class Overall_2(object):
    ...
    def risk_2(self):
        try:
            WebDriverWait(self.driver1,5,1).until(lambda x:self.driver1.find_element_by_xpath('//*[@id="app"]/div/div/div[2]/div[1]/ul/li/ul/li[2]/a').click())
        except:
        ...
        try:
            if name2 == self.lists[5]:
                ...
                print('当前模块错误%s'%(name2))
        except:
            print('未定位到 %s 模块...' %(name2))

        self.action.move_to_element(WebDriverWait(self.driver1,20,2).until(lambda x:self.driver1.find_element_by_xpath('//*[@id="rigthContent"]/div/div/div/div[2]/div[1]/div/i')))
        print('1')
        self.action.move_to_element(self.driver1.find_element_by_xpath('//*[@id="rigthContent"]/div/div/div/div[2]/div[1]/div/i')).click_and_hold().perform()#这里要注意，必须要讲？中的内容展开，才能定位到下面的内容
        #self.action.move_to_element(self.driver1.find_element_by_xpath('//*[@id="rigthContent"]/div/div/div/div[2]/div[1]/div/i')).perform()#这里要注意，必须要讲？中的内容展开，才能定位到下面的内容
        print('2')
